...
INPUT_MODEL_PATH = "/workspace/thundering-birds/logs/training_run_20250425_210906/model_fold0_epoch298_best.pth"  # Replace with your model path
OUTPUT_MODEL_PATH = "/workspace/thundering-birds/model_fold0_epoch298_best.pth"    # Replace with desired output path
NUM_CLASSES = 206                             # Replace with your number of classes
...
class AttentionChannels(nn.Module):
    ...
    def __init__(self, kernel_size=(3, 3), scaling_factors=(5, 20)):
        ...
        self.cfar1 = CFARLayer(kernel_size=kernel_size, scaling_factor=scaling_factors[0])
        self.cfar2 = CFARLayer(kernel_size=kernel_size, scaling_factor=scaling_factors[1])
...
class StandaloneBirdCLEFModel(nn.Module):
    ...
    def __init__(self, num_classes=206, efficientnet_version='efficientnet-b0',
                 kernel_size=(3, 3), cfar_scaling_factors=(0.5, 0.7)):
        super().__init__()
...
def main():
    ...
    try:
        ...
        try:
            ...
            checkpoint = torch.load(INPUT_MODEL_PATH, map_location='cpu', weights_only=True)
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                state_dict = checkpoint['model_state_dict']
            else:
                state_dict = checkpoint
    ...
    try:
        ...
        verify_model = StandaloneBirdCLEFModel(
